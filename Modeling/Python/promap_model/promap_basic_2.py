...
for k in range(len(df)):
    ...
    #buscamos la celda en la que se encuentra el punto
    x_matrix, y_matrix = find_position(xx, yy, x, y, hx, hy)
    print(f'El punto se ubica en {x_matrix, y_matrix} en la malla')
    radio_x = radio_pintar(hx, bw_x)
    radio_y = radio_pintar(hy, bw_y)

    #obtenemos el radio del estampado
    print(f'Radio de pintura X {radio_x} cuadrados')
    print(f'Radio de pintura Y {radio_y} cuadrados')

    #obtenemos los limites del estampado
    x_left, x_right = limites_x(radio_x, x_matrix, xx)
    y_abajo, y_up = limites_y(radio_y, y_matrix, yy)
    print('Limites: ', x_left, x_right, y_abajo, y_up)

    basura = 0


    for x_i in range(x_left, x_right+1):
        for y_j in range(y_abajo, y_up+1):

            print(f'Estoy en la celda x: {x_i} y:{y_j}')
            nodo_x = xx[x_i][0]
            nodo_y = yy[0][y_j]
            print(f'nodo x {nodo_x} nodo y {nodo_y}')
            time_weight = 1 / n_semanas(total_dias, t)
            if linear_distance(nodo_x, x) > bw_x or linear_distance(
                    nodo_y, y) > bw_y:
            #    print('Esta celda está fuera del limite del ancho de banda')
                ...
                cell_weight = 1 / cells_distance(x, y, nodo_x, nodo_y)
                print(f'Pintando la celda x: {x_i} y:{y_j}\n')
            #print('CELL WIGHT: ', cell_weight)
            #print()

            matriz_con_ceros[x_i][y_j] += time_weight * cell_weight
