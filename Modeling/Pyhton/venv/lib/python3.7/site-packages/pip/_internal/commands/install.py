    ...
    def __init__(self, *args, **kw):
        ...
        cmd_opts.add_option(
                ...
                help='Install packages into <dir>. '
                     'By default this will not replace existing files/folders '
                     'in '
                     '<dir>. Use --upgrade to replace existing packages in '
                     '<dir> '
                     'with new versions.'
        )
    ...
    def run(self, options, args):
        ...
        if options.use_user_site:
            ...
            if virtualenv_no_global():
                raise InstallationError(
                        "Can not perform a '--user' install. User "
                        "site-packages "
        ...
        with self._build_session(options) as session:
            ...
            if options.cache_dir and not check_path_owner(options.cache_dir):
                logger.warning(
                        "The directory '%s' or its parent directory is not "
                        "owned "
                ...
                try:
                    ...
                    if build_failures:
                        raise InstallationError(
                                ...
                                " PEP 517 and cannot be installed "
                                "directly".format(
                                        ", ".join(r.name for r in
                                                  build_failures)))
                    ...
                    if os.path.exists(target_item_dir):
                        ...
                        if os.path.islink(target_item_dir):
                            logger.warning(
                                    ...
                                    'a link. Pip will not automatically '
                                    'replace '
    ...
    def _warn_about_conflicts(self, to_install):
        ...
        for project_name in conflicting:
            ...
            for dep_name, dep_version, req in conflicting[project_name]:
                logger.critical(
                        "%s %s has requirement %s, but you'll have %s %s "
                        "which is "
                        "incompatible.",
